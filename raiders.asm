...
BSLDALoop		   			= $84; (c) ; bank switch LDA/loop count
BankStrobeAddrTemp			= $85; (c) ; bank strobe address/temp char
BSJMP		   				= $86; (c) ; bank switch JMP instruction
BSJMPAddr		   			= $87; (c) ; bank switch JMP address
ram_88						= $88; (c) ; zp_88
ram_89						= $89; (c) ; zp_89
...
;---------------------------------------
	lda		objectXPos ,x				 ;4			; get object's horizontal position
	tay								;2		  
...
WarpToMesaSide
	lda		player0YPos					 ;3			* ; Load vertical position of an object (likely thief or Indy)
	...
	lda		#$05					;2		   * ; 2
	sta		indyYPos				  ;3		 * ; Set Indy's vertical position on entry to Mesa Side
	lda		#$50					;2		   * ; 2
	sta		indyXPos				  ;3		 * ; Set Indy's horizontal position on entry
	tsx								;2		   * ; 2
	cpx		#$fe					;2		   * ; 2
...
CheckIfIndyShotOrTouchedByTsetseFlies
	...
	sta		majorEventFlag					  ;3		 * ; 3
	bne		DespawnMissile0					  ;2/3 =  21 * ; unconditional branch
ClearInputBit0ForSpiderRoom
	rol		playerInput					 ;5			* ; Rotate input left, bit 7 ? carry
	sec								;2		   * ; Set carry (overrides carry from rol)
	ror		playerInput					 ;5			* ; Rotate right, carry -> bit 7 (bit 0 lost)
...
DespawnMissile0
	...
	sta		missileYPos					 ;3	  =	  8 * ; Move missile0 offscreen (to y=127)
CheckGrenadeDetonation
	...
	lda		secondsTimer				   ;3		  * ; get seconds time value
	cmp		grenadeDetinationTime					;3		   * ; compare with grenade detination time
	bne		waitTime				   ;2/3		  * ; branch if not time to detinate grenade
	lda		#$a0					;2		   * ; 2
	sta		weaponYPos					;3		   * ; Move bullet/whip offscreen (simulate detonation?)
	sta		majorEventFlag					  ;3   =  23 * ; Trigger major event (explosion happened?)
ApplyGrenadeWallEffect
	...
	lda		#$02					;2		   *
	sta		grenadeUsed					 ;3			* ; Apply penalty (e.g., reduce score)
	ora		entranceRoomState				   ;3		  *
	sta		entranceRoomState				   ;3		  * ; Mark the entrance room as having the grenade opening
	ldx		#$02					;2		   *
...
UpdateFrameAndSecondsTimer
	inc		frameCount			  ;Up the frame counter by 1		; increment frame count
	lda		#$3f					;	 
	and		frameCount			  ;Every 63 frames (?)	  
	bne		firstLineOfVerticalSync					  ;		 ; branch if roughly 60 frames haven't passed
	inc		secondsTimer			  ;Increse the time of day		 ; increment every second
...
;---------------------------------------
	...
	stx		majorEventFlag					  ;3		 * ; Overflowed: zero -> set majorEventFlag to 0
...
Ld44a
	ldy		indyYPos				  ;3		 ; get Indy's vertical position
	cpy		#$7c					;2		   ; 124 dev
...
Ld465
	bit		resetEnableFlag					  ;3		 * ; Check bit 7 of resetEnableFlag
	bmi		Ld46d					;2/3	   * ; If bit 7 is set, skip (reset enabled)
	lda		#$0e					;2		   *
	sta		ram_A2					;3	 =	10 * ; Set Indys state to 0E
Ld46d
	lda		#$80					;2		   *
	sta		resetEnableFlag					  ;3		 * ; Set bit 7 to enable reset logic
...
Ld4a5
	inx								;2		   * ; 2
	stx		player0SpriteHeight					 ;3			* ; Increment and store progression step or counter
	txa								;2		   * ; 2
	sec								;2		   * ; 2
...
Ld4b0
	...
	sta		snakeYPos				   ;3		  * ; Update snakes vertical position
	lda		ram_D4					;3		   * ; 3
	and		#$03					;2		   * ; 2
	tax								;2		   * ; 2
	lda		ram_D4					;3		   * ; 3
...
Ld4e7
	...
	asl								;2		   * ; 2
	asl								;2		   * ; 2
	sta		BSLDALoop				   ;3		  * ; Multiply X by 4 -> store as upper bits of state
	lda		ram_D4					;3		   * ; 3
	and		#$03					;2		   * ; 2
	tax								;2		   * ; 2
	lda		Ldbff,x					;4		   * ; 4
...
Ld501
	...
	tax								;2		   * ; 2
	lda		Ldbfb,x					;4		   * ; 4
	sta		timePieceGfxPointers				   ;3		  * ; Store horizontal movement/frame data
	lda		#$fa					;2		   *
...
Ld532
	...
	ldx		weaponXPos					;3		   * ; get bullet or whip horizontal position
	txa								;2		   * ; 2
	sec								;2		   * ; 2
	sbc		indyXPos				  ;3		 * ; 3
...
Ld57c
	...
	lda		SWCHA					;4		   * ; read joystick values
	sta		BankStrobeAddrTemp					;3		   * ; Store raw joystick input
	and		#$0f					;2		   *
	cmp		#$0f					;2		   *
	...
	ldx		currentScreenId					   ;3		  * ; get the current screen id
	ldy		#$00					;2		   * ; 2
	sty		BSLDALoop				   ;3		  * ; Reset scan index/counter
	beq		Ld599					;2/3 =	34 * ; Unconditional (Y=0, so BNE not taken)
Ld596
	tax								;2		   * ; Transfer A to X (probably to use as an object index or ID)
	inc		BSLDALoop				   ;5	=	7 * ; Increment $84	 a general-purpose counter or index
Ld599
	lda		indyXPos				  ;3		 * ; 3
	pha								;3		   * ; Temporarily store horizontal position
	lda		indyYPos				  ;3		 * ; get Indy's vertical position
	ldy		BSLDALoop				   ;3		  * ; Load current scan/event index
	cpy		#$02					;2		   * ; 2
	bcs		Ld5ac					;2/3	   * ; If index >= 2, store in reverse order
	sta		BSJMP				   ;3		  * ; Vertical position
	pla								;4		   *
	sta		BSJMPAddr				   ;3		  * ; Horizontal position
	jmp		Ld5b1					;3	 =	29 *
	
Ld5ac
	sta		BSJMPAddr				   ;3		  * ; Vertical -> $87
	pla								;4		   *
	sta		BSJMP				   ;3	=  10 * ; Horizontal -> $86
Ld5b1
	ror		BankStrobeAddrTemp					;5		   * ; Rotate player input to extract direction
...
Ld5db
	sty		currentScreenId					   ;3		  * ; Set new screen based on event result
...
Ld5e0
	...
	inc		playerInput					 ;5			* ; Advance to next inventory slot
...
Ld5fa
	...
	cmp		#$01					;2		   * ; 2
	bne		Ld60c					;2/3	   * ; 2
	inc		numberOfBullets					   ;5		  * ; Give Indy 3 bullets
...
Ld60c
	cmp		#$0b					;2		   * ; 2
	bne		Ld61d					;2/3	   * ; 2
	ror		blackMarketState				   ;5		  * ; rotate Black Market state right
	sec								;2		   * ; set carry
	rol		blackMarketState				   ;5		  * ; rotate left to show Indy carrying Shovel
	ldx		#$45					;2		   * ; 2
	stx		player0YPos					 ;3			* ; Set Y-pos for shovel on screen
	ldx		#$7f					;2		   * ; 2
...
Ld627
	...
	and		playerInput					 ;3			* ; ; Clear the USING_GRENADE_OR_PARACHUTE bit from the player input state
	sta		playerInput					 ;3			* ; Store the updated input state
	jmp		Ld777					;3	 =	21 *
	
Ld638
	lda		#$02					;2		   * ; Load the flag indicating item use (grenade/parachute)
	bit		playerInput					 ;3			* ; Check if the flag is already set in player input
	bne		Ld696					;2/3	   * ; If it's already set, skip re-setting (item already active)
	ora		playerInput					 ;3			* ; Otherwise, set the USING_GRENADE_OR_PARACHUTE bit
	sta		playerInput					 ;3			* ; Save the updated input state
	ldx		selectedInventoryId					  ;3		 * ; get the current selected inventory id
...
Ld64c
	ldx		indyYPos				  ;3		 * ; get Indy's vertical position
	stx		weaponYPos					;3		   * ; Set grenade's starting vertical position
	ldy		indyXPos				  ;3		 * ; get Indy horizontal position
	sty		weaponXPos					;3		   * ; Set grenade's starting horizontal position
	lda		secondsTimer				   ;3		  * ; get the seconds timer
	adc		#$04					;2		   * ; increment value by 5...carry set
	sta		grenadeDetinationTime					;3		   * ; detinate grenade 5 seconds from now
...
Ld671
	...
	stx		usingParachuteBonus					  ;3		 * ; Store the parachute usage flag for scoring bonus
...
Ld699
	lda		weaponYPos					;3		   * ; get bullet or whip vertical position
	...
	adc		player0YPos					 ;3			* ; Add reference vertical offset (likely floor or map tile start)
...
Ld6ac
	sta		BSLDALoop				   ;3		  * ; Store the region index calculated from vertical position
	lda		weaponXPos					;3		   * ; get bullet or whip horizontal position
	sec								;2		   * ; 2
	...
	adc		BSLDALoop				   ;3		  * ; Combine with vertical region index to form a unique map zone index
	...
	ldx		weaponYPos					;3		   * ; get bullet or whip vertical position
	dex								;2		   * ; Decrease projectile X by 2  simulate impact offset
	stx		weaponYPos					;3		   *
	stx		indyYPos				  ;3		 * ; Sync Indy's horizontal position to projectiles new position
	ldx		weaponXPos					;3		   *
	dex								;2		   * ; Decrease projectile X by 2  simulate impact offset
	dex								;2		   *
	stx		weaponXPos					;3		   *
	stx		indyXPos				  ;3		 * ; Sync Indy's horizontal position to projectiles new position
...
Ld6ef
	...
	lda		#$0a					;2		   *
	sta		findingArkBonus					 ;3			* ; Set the bonus for having found the Ark
...
Ld6f7
	...
	sta		skipToMesaFieldBonus				   ;3		  * ; set to reduce score by 9 points
	sta		currentScreenId					   ;3		  * ; Change current screen to Mesa Field
	...
	sta		indyXPos				  ;3		 * ; Set Indy's horizontal position
	sta		weaponXPos					;3		   * ; Set projectile's horizontal position (same as Indy)
	lda		#$46					;2		   * ; Fixed vertical position value (start of Mesa Field?)
	sta		indyYPos				  ;3		 * ; Set Indy's vertical position
	sta		weaponYPos					;3		   * ; Set projectile's vertical position
	...
	sta		player0YPos					 ;3			* ; Likely adjusts tile map base Y
...
Ld71e
	...
	bit		WeaponStatus				  ;3		 * ; check bullet or whip status
	bmi		Ld777					;2/3	   * ; branch if bullet active
	ldy		numberOfBullets					   ;3		  * ; get number of bullets remaining
	bmi		Ld777					;2/3	   * ; branch if no more bullets
	dec		numberOfBullets					   ;5		  * ; reduce number of bullets
	ora		#$80					;2		   *
	sta		WeaponStatus				  ;3		 * ; set BULLET_OR_WHIP_ACTIVE bit
	lda		indyYPos				  ;3		 * ; get Indy's vertical position
	adc		#$04					;2		   * ; Offset to spawn bullet slightly above Indy
	sta		weaponYPos					;3		   * ; Set bullet Y position
	lda		indyXPos				  ;3		 *
	adc		#$04					;2		   * ; Offset to spawn bullet slightly ahead of Indy
	sta		weaponXPos					;3		   * ; Set bullet X position
...
Ld767
	...
	clc								;2		   *
	adc		indyXPos				  ;3		 *
	sta		weaponXPos					;3		   * ; Add to Indys current horizontal position
	txa								;2		   * ; 2				 ; Move vertical offset (X) into A
	clc								;2		   * ; 2
	adc		indyYPos				  ;3		 * ; Add to Indys current vertical position
	sta		weaponYPos					;3	 =	20 * ; Set whip strike vertical position
Ld773
...
Ld777
	...
	sta		indyGfxPointers					 ;3			* ; Set Indy's sprite pointer
...
Ld78e
	sta		indyGfxPointers					 ;3			* ; Store sprite pointer (low byte)
	lda		#$0b					;2	 =	 5 * ; Load height for standard Indy sprite
Ld792
	sta		indySpriteHeight				  ;3		 * ; Store sprite height
...
Ld79d
	and		frameCount					 ;3			* ; Use frameCount to time animation updates
	bne		Ld7b2					;2/3	   * ; If result is non-zero, skip sprite update this frame
	lda		#$0b					;2		   * ; Load base sprite height
	clc								;2		   *
	adc		indyGfxPointers					 ;3			* ; Advance to next sprite frame
...
Ld7c3
	...
	ldx		player0YPos					 ;3			* ; Load terrain deformation or sink offset?
	bcs		Ld7e8					;2/3	   * ; If carry is set from earlier BIT, go to advanced sink
	beq		Ld802					;2/3!	   * ; If $DF is 0, no further sinking
	inc		indyYPos				  ;5		 * ; Sink Indy vertically
	inc		weaponYPos					;5		   * ; Sink the projectile too
	and		#$02					;2		   * ; Control sinking frequency
	bne		Ld802					;2/3!	   * ; Skip if odd/even frame constraint not me
	dec		player0YPos					 ;5			* ; Reduce sink counter
	inc		objectYPos					;5		   * ; Possibly animation or game state flag
	inc		missileYPos					 ;5			* ; Sink a visible element (perhaps parachute/missile sprite)
	inc		ram_D2					;5		   * ; Another state tracker
	inc		objectYPos					;5		   *
	inc		missileYPos					 ;5			* ; Repeated to simulate multi-phase sinking
	inc		ram_D2					;5		   * ; 5
...
Ld7e8
	...
	dec		indyYPos				  ;5		 * ; Move Indy upward
	dec		weaponYPos					;5		   * ; Move projectile upward as well
	and		#$02					;2		   * ; Use timing mask to control frame-based rise rate
	bne		Ld802					;2/3!	   * ; If not aligned, skip this update
	inc		player0YPos					 ;5			* ; Increase sink offset counter (reversing descent)
	dec		objectYPos					;5		   * ; Adjust state/animation back
	dec		missileYPos					 ;5			* ; Move visible missile/sprite upward
...
Ld84e
	...
	sta		indyXPos				  ;3		 ; 3
	lda		#$48					;2		   ; 2
	sta		objectXPos					 ;3			; Unknown, likely related to screen offset or trigger state
	lda		#$1f					;2		   ; 2
	sta		indyYPos				  ;3		 ; Set Indy's vertical position in the Ark Room
...
Ld85e
	sta		player0YPos,x				 ;4			* ; Clear/reset memory at $DF$E4
	sta		ram_E0,x				;4		   *
...
ResetRoomFlags
	...
	lda		pickupStatusFlags					;3		   ; Read item collection flags
	stx		pickupStatusFlags					;3		   ; Clear them all (reset pickups for new screen)
	jsr		Ldd59					;6		   ; General-purpose screen initialization/reset routine
	rol		playerInput					 ;5			; Rotate input flags  possibly to mask off an "item use" bit
	clc								;2		   ; 2
	ror		playerInput					 ;5			; Reverse the bit rotation; keeps input state consistent
	ldx		currentScreenId					   ;3		  ; Load which room Indy is in
	lda		Ldb92,x					;4		  
	sta		playfieldControl				   ;3		  ; Set up playfield reflection, score display, priorities
	cpx		#$0d					;2		  
...
Ld8b1
	lda		Ldbee,x					;4		   * ; 4
	sta		player0GfxPointers					;3		   * ; Set low byte of sprite pointer for P0 (non-Indy)
	lda		Ldbe1,x					;4		   * ; 4
	sta		ram_DE					;3		   * ; Set high byte of sprite pointer for P0
	lda		Ldbc9,x					;4		   * ; 4
	sta		player0SpriteHeight					 ;3			* ; Set height of the sprite (e.g., enemy size)
	lda		Ldbd4,x					;4		   * ; 4
	sta		objectXPos					 ;3			* ; Likely a screen property (enemy group type, warp flag)
	lda		Ldc0e,x					;4		   * ; 4
	sta		ram_CA					;3		   * ; Possibly related to object spawning
	lda		Ldc1b,x					;4		   * ; 4
	sta		missileYPos					 ;3			* ; Position for environmental object (missile0 = visual fx)
	cpx		#$0b					;2		   *
	...
	lda		Ldc28,x					;4		   * ; 4
	sta		pf1GfxPointers					 ;3			* ; PF1 low byte
	lda		Ldc33,x					;4		   * ; 4
	sta		ram_E2					;3		   * ; PF1 high byte
	lda		Ldc3e,x					;4		   * ; 4
	sta		pf2GfxPointers					 ;3			* ; PF2 low byte
	lda		Ldc49,x					;4		   * ; 4
	sta		ram_E4					;3		   * ; PF2 high byte
	lda		#$55					;2		   * ; 2
	sta		ram_D2					;3		   * ; Likely a default animation frame or sound cue value
	sta		weaponYPos					;3		   * ; Default vertical position for bullets/whips
	cpx		#$06					;2		   *
	...
	cpx		#$02					;2		   *
	beq		Ld92a					;2/3	   *
	sta		objectYPos					;3	 = 106 * ; Default vertical position for objects (top of screen)
Ld902
	...
	lda		#$ff					;2		   * ; 2
	sta		missileYPos					 ;3	  =	 27 * ; Hide missile object by placing it off-screen
Ld918
	sty		player0YPos					 ;3			* ; Finalize vertical object/environment state
...
Ld91b
	...
	lda		#$1a					;2		   *
	sta		objectYPos					;3		   * ; Set vertical position for the top object
	lda		#$26					;2		   * ; 2
	sta		player0YPos					 ;3			* ; Set vertical position for the bottom object
	rts								;6	 =	24 * ; Return
...
Ld935
	...
	sta		objectYPos					;3		   * ; Set the object's vertical position
...
Ld93e
	...
	ldy		#$40					;2		   * ; 2
	sty		dungeonGfx					;3		   * ; Set visual reference for top-of-dungeon graphics
	bne		Ld958					;2/3 =	20 * ; Always taken
Ld950
	ldy		#$ff					;2		   * ; 2
	sty		dungeonGfx					;3		   * ; Top of dungeon should render with full brightness/effect
	iny								;2		   * ; y = 0
...
Ld958
	...
	sty		snakeYPos				   ;3	=  23 * ; Set snake enemy Y-position baseline
Ld968
	cpx		#$09					;2		   *
	...
	lda		#$50					;2		   * ; 2
	sta		player0YPos					 ;3			* ; Set environmental sink value  starts Indy sinking
	rts								;6	 =	22 * ; return
	
ReturnFromRoomSpecificInit
	lda		#$00					;2		   *
	sta		player0YPos					 ;3			* ; Clear the environmental sink value (Indy won't sink)
...
CheckRoomOverrideCondition
	...
	cpy		BSJMP				   ;3		  * ; Compare with current override key or control flag
...
ApplyVerticalOverride
	sta		indyYPos				  ;3   =   3 * ; Apply vertical override to Indy
ApplyHorizontalOverride
	...
	sta		indyXPos				  ;3   =   9 * ; Apply horizontal override to Indy
...
CheckAdvancedOverrideConditions
	...
	cpy		BSJMPAddr				   ;3		  * ; Compare with current horizontal state
...
Ld9b6
	lda		BSJMPAddr				   ;3		  * ; Load current horizontal position
...
Ld9fe
	...
	cmp		selectedInventoryId					  ;3		 * ; compare with current selected inventory id
...
TakeItemFromInventory
	ldy		numberOfInventoryItems					 ;3			* ; get number of inventory items
...
.takeItemFromInventoryLoop
	cmp		inventoryGraphicPointers,x				  ;4		 * ; Compare target LSB value to current inventory slot
	bne		.checkNextItem					 ;2/3		* ; If not a match, try the next slot
	cpx		selectedInventoryIndex					 ;3			*
	beq		.checkNextItem					 ;2/3		*
	dec		numberOfInventoryItems					 ;5			* ; reduce number of inventory items
	lda		#$00					;2		   *
	sta		inventoryGraphicPointers,x				  ;4		 * ; place empty sprite in inventory
...
.takeSelectedItemFromInventory
	...
	sta		inventoryGraphicPointers,x				  ;4		 * ; remove selected item from inventory
	ldx		selectedInventoryId					  ;3		 * ; get current selected inventory id
	cpx		#$07					;2		   *
...
PlaceItemInInventory
	ldx		numberOfInventoryItems					 ;3			* ; get number of inventory items
...
.searchForEmptySpaceLoop
	ldy		inventoryGraphicPointers,x				  ;4		 * ; get the LSB for the inventory graphic
...
.addInventoryItem
	...
	asl								;2		   *
	asl								;2		   *
	sta		inventoryGraphicPointers,x				  ;4		 * ; place graphic LSB in inventory
	lda		numberOfInventoryItems					 ;3			* ; get number of inventory items
	bne		Ldd0a					;2/3	   * ; branch if Indy carrying items
	stx		selectedInventoryIndex					 ;3			* ; set index to newly picked up item
	sty		selectedInventoryId					  ;3   =  23 * ; set the current selected inventory id
Ldd0a
	inc		numberOfInventoryItems					 ;5			* ; increment number of inventory items
	cpy		#$04					;2		   *
	bcc		Ldd15					;2/3	   * ; 2
...
ShowItemAsNotTaken
	...
	sta		basketItemsStatus					;3		   * ; clear status bit showing item not taken
	rts								;6	 =	26 *
	
.showPickUpItemAsNotTaken
	and		pickupItemsStatus					;3		   *
	sta		pickupItemsStatus					;3		   * ; clear status bit showing item not taken
	rts								;6	 =	12 *
...
ShowItemAsTaken
	...
	sta		basketItemsStatus					;3		   * ; show item taken
	rts								;6	 =	26 *
	
Ldd3e
	ora		pickupItemsStatus					;3		   *
	sta		pickupItemsStatus					;3		   * ; show item taken
	rts								;6	 =	12 *
...
reset_vars
	lda		#$20					;2		   *
	sta		inventoryGraphicPointers				  ;3		 * ; place coins in Indy's inventory
	lsr								;2		   * ; divide value by 8 to get the inventory id
	lsr								;2		   *
	lsr								;2		   *
	sta		selectedInventoryId					  ;3		 * ; set the current selected inventory id
	inc		numberOfInventoryItems					 ;5			* ; increment number of inventory items
	lda		#$00					;2		   *
...
DetermineFinalScore
	lda		adventurePoints					  ;3		 * ; get current adventure points
	sec								;2		   *
	sbc		findingArkBonus					 ;3			* ; reduce for finding the Ark of the Covenant
	sbc		usingParachuteBonus					  ;3		 * ; reduce for using the parachute
	sbc		skipToMesaFieldBonus				   ;3		  * ; reduce if player skipped the Mesa field
	sbc		yarFoundBonus					;3		   * ; reduce if player found Yar
	sbc		livesLeft				   ;3		  * ; reduce by remaining lives
	sbc		usingHeadOfRaInMapRoomBonus					 ;3			* ; reduce if player used the Head of Ra
	sbc		landingInMesaBonus					;3		   * ; reduce if player landed in Mesa
	sbc		unknown_action					;3		   * ; 3
	clc								;2		   *
	adc		grenadeUsed					 ;3			* ; add 2 if Entrance Room opening activated
	adc		escapedShiningLight					  ;3		 * ; add 13 if escaped from Shining Light prison
	adc		thiefShot				   ;3		  * ; add 4 if shot a thief
	sta		adventurePoints					  ;3		 *
	rts								;6	 =	49 *
...
move_enemy
	...
	dec		objectYPos,x			   ;Move enemy left 1 unit ; move object up one pixel
mov_emy_right
	ror								;Rotate next bit into carry
	bcs		mov_emy_down			  ;if 1 check if enemy should go up
	inc		objectYPos,x			   ;Move enemy right 1 unit ; move object down one pixel
mov_emy_down
	ror								;Rotate next bit into carry
	bcs		mov_emy_up				;if 1 check if enemy should go up
	dec		objectXPos ,x				;Move enemy down 1 unit ; move object left one pixel
mov_emy_up
	ror								;Rotate next bit into carry
	bcs		mov_eny_finish			;if 1, moves are finished
	inc		objectXPos ,x				;Move enemy up 1 unit ; move object right one pixel
mov_eny_finish
	rts								;return
...
;--------------------------------------
	sta		HMOVE					;3		   * ; 3 = @03
	lda		(pf1GfxPointers ),y				 ;5			* ; 5
	sta		PF1						;3		   * ; 3 = @11
	lda		(pf2GfxPointers ),y				 ;5			* ; 5
	sta		PF2						;3		   * ; 3 = @19
	bcc		Lf033					;2/3 =	21 * ; 2
Lf01a
	sbc		ram_D4					;3		   * ; 3
...
Lf033
	...
	sec								;2		   * ; 2
	sbc		indyYPos				  ;3		 * ; 3
	cmp		indySpriteHeight				  ;3		 * ; 3
	bcs		Lf079					;2/3	   * ; 2
	tay								;2		   * ; 2
	lda		(indyGfxPointers),y				 ;5			* ; 5
	tax								;2	 =	26 * ; 2
Lf043
	lda		scanline				  ;3		 * ; 3
	sec								;2		   * ; 2
	sbc		objectYPos					;3		   * ; 3
	cmp		player0SpriteHeight					 ;3			* ; 3
	bcs		Lf07d					;2/3	   * ; 2
	tay								;2		   * ; 2
	lda		(player0GfxPointers),y				;5		   * ; 5
	tay								;2	 =	22 * ; 2
Lf050
	lda		scanline				  ;3		 * ; 3
	sta		WSYNC					;3	 =	 6 *
...
;--------------------------------------
	...
	bcs		Lf06e					;2/3	   * ; 2
	tay								;2		   * ; 2
	lda		(timePieceGfxPointers ),y			   ;5		  * ; 5
	sta		ENABL					;3		   * ; 3 = @40
	sta		HMBL					;3	 =	43 * ; 3 = @43
Lf06e
	inc		scanline				  ;5		 * ; 5		  increment scanline
	lda		scanline				  ;3		 * ; 3
	cmp		#$50					;2		   * ; 2
	bcc		Lf003					;2/3	   * ; 2
	jmp		Lf1ea					;3	 =	15 * ; 3
...
Lf09c
	...
	sta		(pf1GfxPointers ,x)				 ;6	  =	 16 * ; 6		  set player 0 color or fine motion
Lf0a4
	inc		scanline				  ;5		 * ; 5		  increment scan line
	ldx		scanline				  ;3		 * ; 3		  get current scan line
	lda		#$02					;2		   * ; 2
	cpx		missileYPos					 ;3			* ; 3
...
;--------------------------------------
	...
	sec								;2		   * ; 2
	sbc		snakeYPos				   ;3		  * ; 3		  subtract Snake vertical position
	cmp		#$10					;2		   * ; 2
	bcs		Lf0ff					;2/3	   * ; 2
	tay								;2		   * ; 2
	cmp		#$08					;2		   * ; 2
	bcc		Lf0fb					;2/3	   * ; 2
	lda		ram_D8					;3		   * ; 3
	sta		timePieceGfxPointers				   ;3	=  26 * ; 3
Lf0ca
	lda		(timePieceGfxPointers ),y			   ;5		  * ; 5
	sta		HMBL					;3	 =	 8 * ; 3 = @34
Lf0ce
...
;--------------------------------------
	...
	lda		(indyGfxPointers),y				 ;5			* ; 5		  get Indy graphic data
...
Lf157
	...
	sta		BSLDALoop				   ;3		  * ; 3
	lda		(timePieceGfxPointers ),y			   ;5		  * ; 5
	sta		BankStrobeAddrTemp					;3		   * ; 3
	cpy		player0SpriteHeight					 ;3			* ; 3
	bcc		Lf174					;2/3	   * ; 2
	lsr		ram_D4					;5	 =	52 * ; 5
Lf174
	jmp		Lf115					;3	 =	 3 * ; 3
...
Lf1b6
	...
	tay								;2		   *
	lda		Lfc40,y					;4		   *
	sta		player0GfxPointers					;3		   * ; set Thief graphic LSB value
	lda		#$44					;2		   *
	sta		timePieceGfxPointers				   ;3		  *
	lda		#$0f					;2		   *
	sta		player0SpriteHeight					 ;3			* ; 3
	lsr		ram_D4					;5		   * ; 5
	jmp		Lf115					;3	 =	33 * ; 3
...
;--------------------------------------
	...
	sta		COLUP0					;3		   ; 3 = @08
	sta		COLUP1					;3		   ; 3 = @11
	lda		selectedInventoryIndex					 ;3			; 3		  get selected inventory index
...
draw_inventory
	ldy		BSLDALoop				   ;3		  ; 3
	lda		(inventoryGraphicPointers),y			  ;5		 ; 5
	sta		GRP0					;3		   ; 3
	sta		WSYNC					;3	 =	14
;---------------------------------------
;--------------------------------------
...
;--------------------------------------
	ldy		#$02					;2		   ; 2
	lda		numberOfInventoryItems					 ;3			; 3		  get number of inventory items
...
Lf314
	...
	bne		Lf2f4					;2/3!=	19 * ; 2
Lf320
	lda		selectedInventoryId					  ;3		 ; get current selected inventory id
	cmp		#$0f					;2		  
	beq		Lf330					;2/3	   ; 2
	cmp		#$02					;2		  
	bne		Lf344					;2/3	   ; 2
...
Lf340
	ldx		selectedInventoryIndex					 ;3			*
	sta		inventoryGraphicPointers,x				  ;4   =   7 * ; 4
Lf344
	lda		#$00					;2		   ; 2
	sta		ram_A3					;3	 =	 5 ; 3
Lf348
...
Lf373
	...
	lda		#$8f					;2		   * ; 2
	sta		weaponYPos					;3		   *
	stx		indySpriteHeight				  ;3		 * ; 3
	bcs		Lf3c5					;2/3 =	48 * ; unconditional branch
Lf398
	sta		frameCount					 ;3			* ; 3
	sec								;2		   * ; 2
	ror		majorEventFlag					  ;5   =  10 * ; 5
Lf39d
	cmp		#$3c					;2		   * ; 2
	bcc		Lf3a9					;2/3	   * ; 2
	bne		Lf3a5					;2/3	   * ; 2
...
Lf3d0
	...
	ldx		selectedInventoryIndex					 ;3			*
	lda		inventoryGraphicPointers,x				  ;4		 * ; get inventory graphic LSB value
	cmp		#$98					;2		   *
...
Lf3f2
	...
	sta		inventoryGraphicPointers,x				  ;4   =  10 * ; set inventory graphic LSB value
Lf3f9
	inx								;2		   *
	inx								;2		   *
	cpx		#$0b					;2		   *
	...
	ldx		#$00					;2	 =	10 *
Lf401
	ldy		inventoryGraphicPointers,x				  ;4		 * ; get inventory graphic LSB value
...
Lf415
	...
	lsr								;2		   *
	lsr								;2		   *
	sta		selectedInventoryId					  ;3		 * ; set selected inventory id
	cpy		#$90					;2		   *
...
Lf4c3
	sta		GRP1					;3		   ; 3 = @27
	lda		indyYPos				  ;3		 ; 3		  get Indy's vertical position
	sta		COLUP1					;3	 =	 9 ; 3 = @33
Lf4c9
...
Lf4d0
	...
	sta		GRP1					;3		   ; 3
	txa								;2		   ; 2		  move scanline to accumulator
	adc		frameCount					 ;3			; 3		  increase value by current frame count
...
;--------------------------------------
	...
	sty		PF1						;3		   ; 3 = @11
	sty		PF2						;3		   ; 3 = @14
	ldx		currentScreenId					   ;3		  ; 3		  get the current screen id
	lda		Lfaac,x					;4		   ; 4
	sta		PF0						;3		   ; 3 = @24
	iny								;2		   ; 2		  y = 0
	sta		WSYNC					;3	 =	29
...
;--------------------------------------
	sta		HMOVE					;3		   ; 3
	ldy		currentScreenId					   ;3		  ; 3		  get the current screen id
	sta		WSYNC					;3	 =	 9
;---------------------------------------
;--------------------------------------
	sta		HMOVE					;3		   ; 3
...
Lf8c6
	bcs		Lf8cc					;2/3	   * ; 2
	inc		objectYPos,x				;6		   * ; 6
	bne		Lf8ce					;2/3 =	10 * ; 2
Lf8cc
	dec		objectYPos,x				;6	 =	 6 * ; 6
Lf8ce
	lda		objectXPos ,x				 ;4			* ; 4
...
Lf8d9
	bcs		Lf8de					;2/3	   * ; 2
	inc		objectXPos ,x				 ;6	  =	  8 * ; 6
Lf8dd
	rts								;6	 =	 6 * ; 6
	
Lf8de
	dec		objectXPos ,x				 ;6			* ; 6
	rts								;6	 =	12 * ; 6
...
Lfcea
	ror								;2		   * ; 2
	bcs		Lfcef					;2/3	   * ; 2
	dec		objectYPos,x				;6	 =	10 * ; 6
Lfcef
	ror								;2		   * ; 2
	bcs		Lfcf4					;2/3	   * ; 2
	inc		objectYPos,x				;6	 =	10 * ; 6
Lfcf4
	ror								;2		   * ; 2
	bcs		Lfcf9					;2/3	   * ; 2
	dec		objectXPos ,x				 ;6	  =	 10 * ; 6
Lfcf9
	ror								;2		   * ; 2
	bcs		Lfcfe					;2/3	   * ; 2
	inc		objectXPos ,x				 ;6	  =	 10 * ; 6
Lfcfe
	rts								;6	 =	 6 * ; 6
